        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuItem in spuSaleAttrList" :key="spuItem.id">
                <dt class="title">选择{{ spuItem.saleAttrName }}</dt>
                <!-- 排他操作 -->
                <dd
                  changepirce="0"
                  :class="{ active: attrValueList.isChecked == true }"
                  ref="attrItem"
                  @click="
                    changeChecked(attrValueList, spuItem.spuSaleAttrValueList)
                  "
                  v-for="attrValueList in spuItem.spuSaleAttrValueList"
            ...
            <div class="cartWrap">
              <div class="controls">
                <input v-model="count" autocomplete="off" class="itxt" />
              ...
              <div class="add">
                <a @click="addInCart()" href="javascript:">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async addInCart() {
      // 获取用户选择的属性
      let arr = this.$refs.attrItem.filter((item) => {
        return item.getAttribute("class") == "active";
      });
      arr.forEach((item) => {
        console.log(this);
        this.goodAttribute.push(item.textContent);
      });
      try {
        await this.$store.dispatch("cartOptions/AddCartList", {
          goodAttribute: this.goodAttribute,
          skuId: this.goodId,
          skuNum: this.count,
    ...
    // 排他操作
    changeChecked(attrValueList, arr) {
      arr.forEach((item) => {
        item.isChecked = false;
      });
      attrValueList.isChecked = true;
    },
  },
  // 接受商品id
  props: ["goodId"],
  mounted() {
    this.$store.dispatch("detailOptions/GetGoodItem", this.goodId);
  },
};
</script>
